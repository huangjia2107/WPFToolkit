...
{
    public class InchRuler : FrameworkElement
    {
        private static readonly Type _typeofSelf = typeof(InchRuler);

        private readonly Pen _pen = new Pen(Brushes.Black, 1.0);
        private readonly DrawingGroup _drawingGroup = new DrawingGroup();
        // 
        private decimal _baseStep = 96;
        ...
        {
            public DeferHelper(InchRuler ruler)
            {
                _ruler = ruler;
            }
...
//             }
        ...
        {
            var currentStepIndex = (stepIndex - Offset * Scale * 96m);
        ...
        private void DrawOffsetRight(DrawingContext dc, decimal currentStep, decimal miniStep, int miniStepCount)
        {
            ...
            for (var stepIndex = Offset * Scale * 96m; stepIndex < (decimal)ActualWidth; stepIndex += miniStep)
            {
                if (stepIndex < 0)
                    continue;
        ...
        private void DrawOffsetLeft(DrawingContext dc, decimal currentStep, decimal miniStep, int miniStepCount)
        {
            ...
            for (var stepIndex = Offset * Scale * 96m; stepIndex >= 0; stepIndex -= miniStep)
            {
